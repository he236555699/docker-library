	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		if [ "$MYSQL_REPLICATION_USER" -a "$MYSQL_REPLICATION_PASSWORD" ]; then
			if ["$MYSQL_MASTER_SERVICE_NAME"]; then
				MASTER_SERVICE_NAME=`echo $MYSQL_MASTER_SERVICE_NAME`
				MASTER_SERVICE_NAME=`echo $MASTER_SERVICE_NAME`
				echo "STOP SLAVE;" | "${mysql[@]}" 
				echo "CHANGE MASTER TO MASTER_HOST='$MASTER_SERVICE_NAME', MASTER_USER='$MYSQL_REPLICATION_USER', MASTER_PASSWORD='$MYSQL_REPLICATION_PASSWORD';" | "${mysql[@]}" 
