...
if [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		if [ "$MYSQL_REPLICATION_USER" -a "$MYSQL_REPLICATION_PASSWORD" ]; then
			...
			echo "CHANGE MASTER TO MASTER_HOST='$MYSQL_MASTER_55_SERVICE_HOST', MASTER_USER='$MYSQL_REPLICATION_USER', MASTER_PORT='$MYSQL_MASTER_55_SERVICE_PORT', MASTER_PASSWORD='$MYSQL_REPLICATION_PASSWORD' ;" | "${mysql[@]}" 
