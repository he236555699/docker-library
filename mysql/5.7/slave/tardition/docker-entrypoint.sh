	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		if [ "$MYSQL_REPLICATION_USER" -a "$MYSQL_REPLICATION_PASSWORD" ]; then
			if ["$MYSQL_MASTER_SERVICE_NAME"]; then
				...
				echo "CHANGE MASTER TO MASTER_HOST='$MYSQL_MASTER_SERVICE_NAME', MASTER_USER='$MYSQL_REPLICATION_USER', MASTER_PASSWORD='$MYSQL_REPLICATION_PASSWORD', MASTER_AUTO_POSITION=1;" | "${mysql[@]}" 
